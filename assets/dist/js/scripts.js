...
$.ajax({
    ...
    success: function (json) {
        ...
        if (rssFeed != null) {
            const proxyUrl = 'https://rss-proxy-api.netlify.app/.netlify/functions/fetch-xml?url=' + encodeURIComponent(rssFeed.url);

            function loadFeed(url, isFallback = false) {
                $.ajax({
                    async: false,
                    global: false,
                    url: url,
                    ...
                    success: function (data) {
                        ...
                        $xml.find("item").each(function () {
                            ...
                            $('#cards').append(
                                ...
                                (rssFeed.id == 0 || rssFeed.id == 4.5 || rssFeed.id == 5.1 || rssFeed.id == 6.1 || rssFeed.id == 6.2 || rssFeed.id == 6.3 || rssFeed.id == 6.4 ?
                                    ""
                                    :
                                ...
                                '<p class=\"card-text\">' + (rssFeed.id == 4.5 || rssFeed.id == 5.1 ? item.contentEncoded : item.description) + '</p>' +
                    ...
                    error: function (xhr, status, error) {
                        if (!isFallback) {
                            console.warn('Proxy failed. Attempting direct fetch for: ' + rssFeed.url);
                            loadFeed(rssFeed.url, true);
                        } else {
